...
srcdir=exp/nnet3/lstm_3_ivec_ld5 #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
# train_data_dir=data/tr05_multi_noisy #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
train_data_dir=$data_fmllr/tr05_multi_noisy
online_ivector_dir=exp/nnet3/ivectors_tr05_multi_noisy #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
if [ $stage -le 1 ]; then
  ...
  nj=5 # have a high number of jobs because this could take a while, and we might
...
if [ -z "$lats_dir" ]; then
  ...
  if [ $stage -le 2 ]; then
    ...
    num_threads_denlats=1 #10
    subsplit=1 #40 # number of jobs that run per job (but 2 run at a time, so total jobs is 80, giving
...
if [ $stage -le 5 ]; then
  for x in `seq $decode_start_epoch $num_epochs`; do
    ...
    for lm_suffix in tgpr bd_tgpr; do
      ...
      for data in tr05_multi_noisy et05_simu_beamformit_5mics et05_real_beamformit_5mics dt05_simu_beamformit_5mics dt05_real_beamformit_5mics; do
        steps/nnet3/decode.sh --nj 4 --cmd "$decode_cmd" \
            --online-ivector-dir exp/nnet3/ivectors_${data} \
           $graph_dir data/${data} $dir/decode_lstmd_${data} || exit 1;
      done
    done
  done
fi
