...
# N-best rescoring using a LSTMLM
if [ $stage -le 5 ]; then
  # check the best lmw
  if [ ! -f $dir/log/best_wer_${enhan}_${lm_suffix} ]; then
    echo "error, rescoring with a high-order n-gram seems to be failed"
  ...
  for t in $tasks; do
    steps/rnnlmrescore.sh --inv-acwt $lmw --N $nbest --use-phi true \
      $rnnweight \
      $mdir/data/lang_test_${lm_suffix} \
      $mdir/data/lang_test_${lstmlm_suffix} \
      data-fmllr-tri3b/${t}_$enhan \
      $dir/decode_tgpr_5k_${t}_${enhan}_${lm_suffix} \
      $dir/decode_tgpr_5k_${t}_${enhan}_${lstmlm_suffix}_w${lstmweight}_n${nbest}
  done
  # calc wers for LSTM results
  local/chime4_calc_wers.sh $dir ${enhan}_${lstmlm_suffix}_w${lstmweight}_n${nbest} $dir/graph_tgpr_5k \
      > $dir/best_wer_${enhan}_${lstmlm_suffix}_w${lstmweight}_n${nbest}.result
  head -n 15 $dir/best_wer_${enhan}_${lstmlm_suffix}_w${lstmweight}_n${nbest}.result
fi
