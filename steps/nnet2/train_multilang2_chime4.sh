...
cmd=run.pl
num_epochs=10      # Number of epochs of training (for first language); 10
...
num_threads=1  # default suitable for CPU-based training
    ...
    for lang in $(seq 0 $[$num_lang-1]); do
      ...
      if [ $x -gt 0 ] && [ ! -f $dir/$lang/log/mix_up.$[$x-1].log ]; then
        $cmd $dir/$lang/log/progress.$x.log \
          nnet-show-progress --use-gpu=yes $dir/$lang/$[$x-1].mdl $dir/$lang/$x.mdl \
...
if [ $stage -le $num_iters ]; then
  ...
  for lang in $(seq 0 $[$num_lang-1]); do
    ...
    $cmd $combine_parallel_opts $dir/$lang/log/combine.log \
      nnet-combine-fast --initial-model=100000 --num-lbfgs-iters=40 --use-gpu=yes \
