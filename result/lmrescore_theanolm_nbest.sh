...
dataDir="/home/rezki/sandbox/kaldi/egs/chime4DevFE1_allChannels3/s5_6ch/data-fmllr-tri3b/${task}_beamformit_5mics"
echo local/scoreTheanoLM.sh --cmd "$cmd" $dataDir $lang_dir $out_dir


script_name=$(basename "${0}")
int2sym="utils/int2sym.pl"
compute_scores="theanolm_compute_scores_.sh"
...
# # ==============score theanolm start
local/scoreTheanoLM.sh --cmd "$cmd" $dataDir $lang_dir $out_dir ||
...
then
	...
	echo "JOB=1:${nj}" "${out_dir}/log/theanolm_compute_scores.JOB.log" \
      "${compute_scores}" \
        "${nnlm}" \
        "${archives_dir}/JOB/temp" \
        "${archives_dir}/JOB/words_text" \
        "${archives_dir}/JOB/scores.nnlm"
    #chimeEgsDir = "/home/rezki/sandbox/kaldi/egs/chime4Dev/s5_6ch"
